...
#define STRNLEN(soname, fnname) \
   ...
   { \
      SizeT i = 0; \
      while (i < n && str[i] != 0) i++; \
      if (VALGRIND_CHECK_MEM_IS_UNTAINTED(str, i)) \
        VALGRIND_MAKE_MEM_TAINTED(&i, sizeof(i)); \
      return i; \
   }
...
// XXX: note - we taint the return value because it is directly derived
//      from input (if the str is tainted).  This is a kludge
...
// XXX: taint checking using nmax may introduce false positives if either string
//      is shorter.
#define STRNCMP(soname, fnname) \
   ...
   { \
      SizeT n = 0; \
      int r = 0; \
      const char *orig_s1 = s1, *orig_s2 = s2; \
      \
      while (True) { \
         if (n >= nmax) break; \
         if (*s1 == 0 && *s2 == 0) break; \
         ...
         if (*s2 == 0) { r = 1; break; } \
\
         if (*(unsigned char*)s1 < *(unsigned char*)s2) { r = -1; break; } \
         if (*(unsigned char*)s1 > *(unsigned char*)s2) { r = 1;  break; } \
 \
         s1++; s2++; n++; \
      } \
      if (VALGRIND_CHECK_MEM_IS_UNTAINTED(orig_s1, n)) \
        VALGRIND_MAKE_MEM_TAINTED(&r, sizeof(r)); \
      else if (VALGRIND_CHECK_MEM_IS_UNTAINTED(orig_s2, n)) \
        VALGRIND_MAKE_MEM_TAINTED(&r, sizeof(r)); \
      return r; \
   }
...
\
      if ((unsigned char)c1 < (unsigned char)c2) r = -1; \
      else if ((unsigned char)c1 > (unsigned char)c2) r = 1; \
\
      if (VALGRIND_CHECK_MEM_IS_UNTAINTED(orig_s1, len)) \
        VALGRIND_MAKE_MEM_TAINTED(&r, sizeof(r)); \
      else if (VALGRIND_CHECK_MEM_IS_UNTAINTED(orig_s2, len)) \
        VALGRIND_MAKE_MEM_TAINTED(&r, sizeof(r)); \
      return r; \
   }
...
#define MEMCMP(soname, fnname) \
 ...
 \
      ...
      if (VALGRIND_CHECK_MEM_IS_UNTAINTED(s1V, n_orig)) \
        VALGRIND_MAKE_MEM_TAINTED(&res, sizeof(res)); \
      else if (VALGRIND_CHECK_MEM_IS_UNTAINTED(s2V, n_orig)) \
        VALGRIND_MAKE_MEM_TAINTED(&res, sizeof(res)); \
      return res; \
   }
...
/* AIX's libm contains a sqrt implementation which does a nasty thing:
   ...
   This fools flayer completely; the only solution I can think of is
