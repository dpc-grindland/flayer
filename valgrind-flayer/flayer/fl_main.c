...
/*--------------------------------------------------------------------*/
/*--- Flayer: Maintain bitmaps of memory, tracking the           ---*/
/*--- taintedness of each byte.      ---*/
/*---                                                    fl_main.c ---*/
/*--------------------------------------------------------------------*/
...
#include "pub_tool_hashtable.h"     // For fl_include.h
...
#include "flayer.h"   /* for client requests */
...
/* Conceptually, every byte value has 8 V bits, which track whether Memcheck
   ...
   While accessibility isn't important for memcheck, there is little reason
   to remove it at this point.  Currently, it allows easy taint removal.
*/
...
static SecMap* copy_for_writing ( SecMap* dist_sm )
{
   ...
   if (new_sm == NULL)
      VG_(out_of_memory_NORETURN)( "flayer:allocate new SecMap", 
                                   sizeof(SecMap) );
...
Bool get_vbits8 ( Addr a, UChar* vbits8 )
{
   ...
   if      ( VA_BITS2_UNTAINTED   == vabits2 ) { *vbits8 = V_BITS8_UNTAINTED;   }
      ...
      *vbits8 = V_BITS8_UNTAINTED;    // Make V bits defined!
      //ok = False; // XXX: should be fine no?
   } else {
...
// (PDBs) that are represented by VA_BITS2_PARTUNTAINTED in the main shadow
// memory.
//
...
// This must be a power of two;  this is checked in fl_pre_clo_init().
...
static void gcSecVBitTable(void)
{
   ...
   if (VG_(clo_verbosity) > 1) {
      ...
      VG_(message)(Vg_DebugMsg, "flayer GC: %d nodes, %d survivors (%s)",
   ...
   if (n_survivors > (secVBitLimit * MAX_SURVIVOR_PROPORTION)) {
      ...
      if (VG_(clo_verbosity) > 1)
         VG_(message)(Vg_DebugMsg, "flayer GC: increase table size to %d",
                      secVBitLimit);
   }
}
...
// Forward declarations
static void fl_record_address_error  ( ThreadId tid, Addr a,
                                       Int size, Bool isWrite );
static void fl_record_core_mem_error ( ThreadId tid, Bool isAddrErr, Char* s );
static void fl_record_regparam_error ( ThreadId tid, Char* msg );
static void fl_record_memparam_error ( ThreadId tid, Addr a,
                                       Bool isAddrErr, Char* msg );
static void fl_record_jump_error     ( ThreadId tid, Addr a );

static
#ifndef PERF_FAST_LOADV
INLINE
#endif
ULong fl_LOADVn_slow ( Addr a, SizeT nBits, Bool bigendian )
{
   ...
   */
   if (EXPECTED_TAKEN(sizeof(void*) == 8 
      ...
      if (EXPECTED_TAKEN(vabits16 == VA_BITS16_UNTAINTED))
         return V_BITS64_UNTAINTED;
      if (EXPECTED_TAKEN(vabits16 == VA_BITS16_TAINTED))
   ...
   if (EXPECTED_TAKEN(sizeof(void*) == 8 
      ...
      if (EXPECTED_TAKEN(vabits8 == VA_BITS8_UNTAINTED))
         return ((UWord)0xFFFFFFFF00000000ULL | (UWord)V_BITS32_UNTAINTED);
      if (EXPECTED_TAKEN(vabits8 == VA_BITS8_TAINTED))
   ...
   for (i = szB-1; i >= 0; i--) {
      PROF_EVENT(31, "fl_LOADVn_slow(loop)");
   ...
   */
   partial_load_exemption_applies
      = FL_(clo_partial_loads_ok) && szB == VG_WORDSIZE 
                                   && VG_IS_WORD_ALIGNED(a) 
                                   && n_addrs_bad < VG_WORDSIZE;

   if (n_addrs_bad > 0 && !partial_load_exemption_applies)
      fl_record_address_error( VG_(get_running_tid)(), a, szB, False );
...
INLINE
#endif
void fl_STOREVn_slow ( Addr a, SizeT nBits, ULong vbytes, Bool bigendian )
{
   SizeT szB = nBits / 8;
   ...
   */
   if (EXPECTED_TAKEN(sizeof(void*) == 8 
      ...
      if (EXPECTED_TAKEN( !is_distinguished_sm(sm) && 
         ...
         if (EXPECTED_TAKEN(V_BITS64_UNTAINTED == vbytes)) {
   ...
   if (EXPECTED_TAKEN(sizeof(void*) == 8
      ...
      if (EXPECTED_TAKEN( !is_distinguished_sm(sm) && 
         ...
         /* is mapped, and is addressible. */
         // Convert full V-bits in register to compact 2-bit form.
         if (EXPECTED_TAKEN(V_BITS32_UNTAINTED == (vbytes & 0xFFFFFFFF))) {
   ...
   for (i = 0; i < szB; i++) {
      PROF_EVENT(36, "fl_STOREVn_slow(loop)");
      ai     = a + byte_offset_w(szB, bigendian, i);
      vbits8 = vbytes & 0xff;
   ...
   if (n_addrs_bad > 0)
      fl_record_address_error( VG_(get_running_tid)(), a, szB, True );
}
                                      ...
                                      UWord dsm_num )
{
   ...
   if (lenT > 100 * 1000 * 1000) {
      if (VG_(clo_verbosity) > 0 && !VG_(clo_xml)) {
         ...
         if (vabits16 == VA_BITS16_TAINTED) s = "undefined";
         if (vabits16 == VA_BITS16_UNTAINTED  ) s = "defined";
...
void FL_(make_mem_noaccess) ( Addr a, SizeT len )
{
   PROF_EVENT(40, "FL_(make_mem_noaccess)");
   DEBUG("FL_(make_mem_noaccess)(%p, %lu)\n", a, len);
...
void FL_(make_mem_undefined) ( Addr a, SizeT len )
{
   PROF_EVENT(41, "FL_(make_mem_undefined)");
   DEBUG("FL_(make_mem_undefined)(%p, %lu)\n", a, len);
   set_address_range_perms ( a, len, VA_BITS16_TAINTED, SM_DIST_TAINTED );
}

void FL_(make_mem_defined) ( Addr a, SizeT len )
{
   PROF_EVENT(42, "FL_(make_mem_defined)");
   DEBUG("FL_(make_mem_defined)(%p, %lu)\n", a, len);
   set_address_range_perms ( a, len, VA_BITS16_UNTAINTED, SM_DIST_UNTAINTED );
}
...
/* For each byte in [a,a+len), if the byte is addressable, make it be
   ...
   words a version of FL_(make_mem_defined) that doesn't mess with
...
static void make_mem_defined_if_addressable ( Addr a, SizeT len )
{
   ...
   for (i = 0; i < len; i++) {
      ...
      if (EXPECTED_TAKEN(VA_BITS2_NOACCESS != vabits2)) {
         set_vabits2(a+i, VA_BITS2_UNTAINTED);
      }
   }
}
...
void FL_(copy_address_range_state) ( Addr src, Addr dst, SizeT len )
{
   SizeT i, j;
   UChar vabits2, vabits8;
   Bool  aligned, nooverlap;

   DEBUG("FL_(copy_address_range_state)\n");
   PROF_EVENT(50, "FL_(copy_address_range_state)");
   ...
   if (nooverlap && aligned) {
      ...
      while (len >= 4) {
         ...
         if (EXPECTED_TAKEN(VA_BITS8_UNTAINTED == vabits8 
                            || VA_BITS8_TAINTED == vabits8 
      ...
      if (src < dst) {
         for (i = 0, j = len-1; i < len; i++, j--) {
            PROF_EVENT(51, "FL_(copy_address_range_state)(loop)");
      ...
      if (src > dst) {
         for (i = 0; i < len; i++) {
            PROF_EVENT(52, "FL_(copy_address_range_state)(loop)");
            vabits2 = get_vabits2( src+i );
            set_vabits2( dst+i, vabits2 );
            if (VA_BITS2_PARTUNTAINTED == vabits2) {
               set_sec_vbits8( dst+i, get_sec_vbits8( src+i ) );
...
static INLINE
void make_aligned_word32_defined ( Addr a )
{
   UWord   sm_off;
   SecMap* sm;

   PROF_EVENT(300, "make_aligned_word32_defined");

#ifndef PERF_FAST_STACK2
   FL_(make_mem_defined)(a, 4);
#else
   if (EXPECTED_NOT_TAKEN(a > MAX_PRIMARY_ADDRESS)) {
      PROF_EVENT(301, "make_aligned_word32_defined-slow1");
      FL_(make_mem_defined)(a, 4);
      return;
   }
...
void make_aligned_word64_defined ( Addr a )
{
   ...
   PROF_EVENT(320, "make_aligned_word64_defined");

#ifndef PERF_FAST_STACK2
   FL_(make_mem_defined)(a, 8);
#else
   if (EXPECTED_NOT_TAKEN(a > MAX_PRIMARY_ADDRESS)) {
      PROF_EVENT(321, "make_aligned_word64_defined-slow1");
      FL_(make_mem_defined)(a, 8);
      return;
   }
   ...
   sm       = get_secmap_for_writing_low(a);
   sm_off16 = SM_OFF_16(a);
   ((UShort*)(sm->vabits8))[sm_off16] = VA_BITS16_UNTAINTED;
#endif
}
...
void make_aligned_word64_undefined ( Addr a )
{
   ...
   sm       = get_secmap_for_writing_low(a);
   sm_off16 = SM_OFF_16(a);
   ((UShort*)(sm->vabits8))[sm_off16] = VA_BITS16_TAINTED;
#endif
}
...
void make_aligned_word64_noaccess ( Addr a )
{
   ...
   sm       = get_secmap_for_writing_low(a);
   sm_off16 = SM_OFF_16(a);
   //((UShort*)(sm->vabits8))[sm_off16] = VA_BITS16_NOACCESS|VA_BITS16_UNTAINTED;
...
static void VG_REGPARM(1) fl_new_mem_stack_4(Addr new_SP)
{
   PROF_EVENT(110, "new_mem_stack_4");
   if (VG_IS_4_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      make_aligned_word32_defined ( -VG_STACK_REDZONE_SZB + new_SP );
   } else {
      FL_(make_mem_defined) ( -VG_STACK_REDZONE_SZB + new_SP, 4 );
   }
}

static void VG_REGPARM(1) fl_die_mem_stack_4(Addr new_SP)
{
   PROF_EVENT(120, "die_mem_stack_4");
   if (VG_IS_4_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      make_aligned_word32_noaccess ( -VG_STACK_REDZONE_SZB + new_SP-4 );
   } else {
      FL_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-4, 4 );
   }
}

static void VG_REGPARM(1) fl_new_mem_stack_8(Addr new_SP)
{
   PROF_EVENT(111, "new_mem_stack_8");
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP );
   } else if (VG_IS_4_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      make_aligned_word32_defined ( -VG_STACK_REDZONE_SZB + new_SP   );
      make_aligned_word32_defined ( -VG_STACK_REDZONE_SZB + new_SP+4 );
   } else {
      FL_(make_mem_defined) ( -VG_STACK_REDZONE_SZB + new_SP, 8 );
   }
}

static void VG_REGPARM(1) fl_die_mem_stack_8(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FL_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-8, 8 );
   }
}

static void VG_REGPARM(1) fl_new_mem_stack_12(Addr new_SP)
{
   PROF_EVENT(112, "new_mem_stack_12");
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP   );
      make_aligned_word32_defined ( -VG_STACK_REDZONE_SZB + new_SP+8 );
   } else if (VG_IS_4_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      make_aligned_word32_defined ( -VG_STACK_REDZONE_SZB + new_SP   );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+4 );
   } else {
      FL_(make_mem_defined) ( -VG_STACK_REDZONE_SZB + new_SP, 12 );
   }
}

static void VG_REGPARM(1) fl_die_mem_stack_12(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP-12 )) {
      ...
      FL_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-12, 12 );
   }
}

static void VG_REGPARM(1) fl_new_mem_stack_16(Addr new_SP)
{
   PROF_EVENT(113, "new_mem_stack_16");
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      /* Have 8-alignment at +0, hence do 8 at +0 and 8 at +8. */
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP   );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+8 );
   } else if (VG_IS_4_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      /* Have 4 alignment at +0 but not 8; hence 8 must be at +4.
         Hence do 4 at +0, 8 at +4, 4 at +12. */
      make_aligned_word32_defined ( -VG_STACK_REDZONE_SZB + new_SP    );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+4  );
      make_aligned_word32_defined ( -VG_STACK_REDZONE_SZB + new_SP+12 );
   } else {
      FL_(make_mem_defined) ( -VG_STACK_REDZONE_SZB + new_SP, 16 );
   }
}

static void VG_REGPARM(1) fl_die_mem_stack_16(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FL_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-16, 16 );
   }
}

static void VG_REGPARM(1) fl_new_mem_stack_32(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP    );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+8  );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+16 );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+24 );
   } else if (VG_IS_4_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      /* 8 alignment must be at +4.  Hence do 8 at +4,+12,+20 and 4 at
         +0,+28. */
      make_aligned_word32_defined ( -VG_STACK_REDZONE_SZB + new_SP    );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+4  );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+12 );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+20 );
      make_aligned_word32_defined ( -VG_STACK_REDZONE_SZB + new_SP+28 );
   } else {
      FL_(make_mem_defined) ( -VG_STACK_REDZONE_SZB + new_SP, 32 );
   }
}

static void VG_REGPARM(1) fl_die_mem_stack_32(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FL_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-32, 32 );
   }
}

static void VG_REGPARM(1) fl_new_mem_stack_112(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+16 );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+24 );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+32 );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+40 );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+48 );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+56 );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+64 );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+72 );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+80 );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+88 );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+96 );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+104);
   } else {
      FL_(make_mem_defined) ( -VG_STACK_REDZONE_SZB + new_SP, 112 );
   }
}

static void VG_REGPARM(1) fl_die_mem_stack_112(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FL_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-112, 112 );
   }
}

static void VG_REGPARM(1) fl_new_mem_stack_128(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+96 );
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+104);
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+112);
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+120);
   } else {
      FL_(make_mem_defined) ( -VG_STACK_REDZONE_SZB + new_SP, 128 );
   }
}

static void VG_REGPARM(1) fl_die_mem_stack_128(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FL_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-128, 128 );
   }
}

static void VG_REGPARM(1) fl_new_mem_stack_144(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+112);
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+120);
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+128);
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+136);
   } else {
      FL_(make_mem_defined) ( -VG_STACK_REDZONE_SZB + new_SP, 144 );
   }
}

static void VG_REGPARM(1) fl_die_mem_stack_144(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FL_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-144, 144 );
   }
}

static void VG_REGPARM(1) fl_new_mem_stack_160(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+128);
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+136);
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+144);
      make_aligned_word64_defined ( -VG_STACK_REDZONE_SZB + new_SP+152);
   } else {
      FL_(make_mem_defined) ( -VG_STACK_REDZONE_SZB + new_SP, 160 );
   }
}

static void VG_REGPARM(1) fl_die_mem_stack_160(Addr new_SP)
{
   ...
   if (VG_IS_8_ALIGNED( -VG_STACK_REDZONE_SZB + new_SP )) {
      ...
      FL_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + new_SP-160, 160 );
   }
}

static void fl_new_mem_stack ( Addr a, SizeT len )
{
   PROF_EVENT(115, "new_mem_stack");
   // nothing starts as tainted/undef when using undef-as-taint
   FL_(make_mem_defined) ( -VG_STACK_REDZONE_SZB + a, len );
}

static void fl_die_mem_stack ( Addr a, SizeT len )
{
   PROF_EVENT(125, "die_mem_stack");
   FL_(make_mem_noaccess) ( -VG_STACK_REDZONE_SZB + a, len );
}
...
*/
void FL_(helperc_MAKE_STACK_UNINIT) ( Addr base, UWord len )
{
   ...
   if (EXPECTED_TAKEN( len == 128 && VG_IS_8_ALIGNED(base) )) {
      ...
      if (a_hi < MAX_PRIMARY_ADDRESS) {
         ...
         if (EXPECTED_TAKEN(sm == sm_hi)) {
            ...
            p[ 0] = VA_BITS16_UNTAINTED;
            p[ 1] = VA_BITS16_UNTAINTED;
            p[ 2] = VA_BITS16_UNTAINTED;
            p[ 3] = VA_BITS16_UNTAINTED;
            p[ 4] = VA_BITS16_UNTAINTED;
            p[ 5] = VA_BITS16_UNTAINTED;
            p[ 6] = VA_BITS16_UNTAINTED;
            p[ 7] = VA_BITS16_UNTAINTED;
            p[ 8] = VA_BITS16_UNTAINTED;
            p[ 9] = VA_BITS16_UNTAINTED;
            p[10] = VA_BITS16_UNTAINTED;
            p[11] = VA_BITS16_UNTAINTED;
            p[12] = VA_BITS16_UNTAINTED;
            p[13] = VA_BITS16_UNTAINTED;
            p[14] = VA_BITS16_UNTAINTED;
            p[15] = VA_BITS16_UNTAINTED;
            return;
         }
      }
   }
   ...
   if (EXPECTED_TAKEN( len == 288 && VG_IS_8_ALIGNED(base) )) {
      ...
      if (a_hi < MAX_PRIMARY_ADDRESS) {
         ...
         if (EXPECTED_TAKEN(sm == sm_hi)) {
            ...
            p[14] = VA_BITS16_UNTAINTED;
            p[15] = VA_BITS16_UNTAINTED;
            p[16] = VA_BITS16_UNTAINTED;
            p[17] = VA_BITS16_UNTAINTED;
            p[18] = VA_BITS16_UNTAINTED;
            p[19] = VA_BITS16_UNTAINTED;
            p[20] = VA_BITS16_UNTAINTED;
            p[21] = VA_BITS16_UNTAINTED;
            p[22] = VA_BITS16_UNTAINTED;
            p[23] = VA_BITS16_UNTAINTED;
            p[24] = VA_BITS16_UNTAINTED;
            p[25] = VA_BITS16_UNTAINTED;
            p[26] = VA_BITS16_UNTAINTED;
            p[27] = VA_BITS16_UNTAINTED;
            p[28] = VA_BITS16_UNTAINTED;
            p[29] = VA_BITS16_UNTAINTED;
            p[30] = VA_BITS16_UNTAINTED;
            p[31] = VA_BITS16_UNTAINTED;
            p[32] = VA_BITS16_UNTAINTED;
            p[33] = VA_BITS16_UNTAINTED;
            p[34] = VA_BITS16_UNTAINTED;
            p[35] = VA_BITS16_UNTAINTED;
            return;
         }
      }
   }
...
/* Returns True if [a .. a+len) is not addressible.  Otherwise,
   ...
   similar. */
Bool FL_(check_mem_is_noaccess) ( Addr a, SizeT len, Addr* bad_addr )
{
   SizeT i;
   UWord vabits2;
...
static FL_ReadResult is_mem_defined ( Addr a, SizeT len, Addr* bad_addr )
{
   SizeT i;
   UWord vabits2;
   ...
   for (i = 0; i < len; i++) {
      ...
      vabits2 = get_vabits2(a);
      if (VA_BITS2_UNTAINTED != vabits2) {
         // Error!  Nb: Report addressability errors in preference to
         // definedness errors.  And don't report definedeness errors unless
         // --undef-value-errors=yes.

         // XXX: in flayer, unaddressable items are not usually tainted. Clean this up later
         if (bad_addr != NULL) *bad_addr = a;
         return FL_ValueErr;
         /*
         if      ( VA_BITS2_TAINTED == vabits2 ) return FL_ValueErr;
         else if  ( VA_BITS2_NOACCESS == vabits2 ) return FL_Ok; //return FL_AddrErr;
         else                                     return FL_Ok;
         */
      }
      a++;
...
static Bool fl_is_defined_asciiz ( Addr a, Addr* bad_addr )
{
   UWord vabits2;

   PROF_EVENT(66, "fl_is_defined_asciiz");
   DEBUG("fl_is_defined_asciiz\n");
   while (True) {
      PROF_EVENT(67, "fl_is_defined_asciiz(loop)");
      vabits2 = get_vabits2(a);
      if (VA_BITS2_UNTAINTED != vabits2) {
         // Error!  Nb: Report addressability errors in preference to
         // definedness errors.
         ...
         /*
         if      ( VA_BITS2_TAINTED == vabits2 ) return FL_ValueErr; 
         else if      ( VA_BITS2_NOACCESS == vabits2 ) return FL_Ok; // FL_AddrErr; ignore for now
         else                                     return FL_ValueErr;
         */
      }
...
static
void fl_pre_syscall(ThreadId tid, UInt syscallno) 
{
}

static
void fl_post_syscall(ThreadId tid, UInt syscallno, SysRes res) 
{
  switch (syscallno) {
#ifdef __NR_read
    case __NR_read:
      FL_(syscall_read)(tid, res);
      break;
#else
# warn __NR_read not defined. No I/O tainting will be possible!
#endif
#ifdef __NR_close
    case __NR_close:
      FL_(syscall_close)(tid, res);
      break;
#else
# warn __NR_close not defined. No FD untainting will be possible!
#endif
#ifdef __NR_open
    case __NR_open:
      FL_(syscall_open)(tid, res);
      break;
#else
# warn __NR_open not defined. No file tainting will be possible!
#endif

#ifdef __NR_socketcall
    case __NR_socketcall:
      FL_(syscall_socketcall)(tid, res);
      break;
#endif
#ifdef __NR_accept
    case __NR_accept:
                                ...
                                Addr base, SizeT size )
{
   ...
   if (!ok) {
      switch (part) {
      case Vg_CoreSysCall:
         fl_record_memparam_error ( tid, bad_addr, /*isAddrErr*/True, s );
         break;

      case Vg_CoreSignal:
         fl_record_core_mem_error( tid, /*isAddrErr*/True, s );
         break;
...
{     
   Addr bad_addr;
   FL_ReadResult res = is_mem_defined ( base, size, &bad_addr );

   if (FL_Ok != res) {
      Bool isAddrErr = ( FL_AddrErr == res ? True : False );

      switch (part) {
      case Vg_CoreSysCall:
         fl_record_memparam_error ( tid, bad_addr, isAddrErr, s );
         break;
      
      /* If we're being asked to jump to a silly address, record an error 
         message before potentially crashing the entire system. */
      case Vg_CoreTranslate:
         fl_record_jump_error( tid, bad_addr );
         break;
                                   ...
                                   Char* s, Addr str )
{
   ...
   res = fl_is_defined_asciiz ( (Addr)str, &bad_addr );
   if (FL_Ok != res) {
      Bool isAddrErr = ( FL_AddrErr == res ? True : False );
      fl_record_memparam_error ( tid, bad_addr, isAddrErr, s );
   }
...
static
void fl_new_mem_startup( Addr a, SizeT len, Bool rr, Bool ww, Bool xx )
{
   ...
   // we can't avoid it.
   DEBUG("fl_new_mem_startup(%p, %llu, rr=%u, ww=%u, xx=%u)\n",
         a, (ULong)len, rr, ww, xx);
   FL_(make_mem_defined)(a, len);
}

static
void fl_new_mem_mmap ( Addr a, SizeT len, Bool rr, Bool ww, Bool xx )
{
   FL_(make_mem_defined)(a, len);
}

static
void fl_post_mem_write(CorePart part, ThreadId tid, Addr a, SizeT len)
{
   FL_(make_mem_defined)(a, len);
}
...
*/
static void fl_post_reg_write ( CorePart part, ThreadId tid, 
...
static 
void fl_post_reg_write_clientcall ( ThreadId tid, 
                                    OffT offset, SizeT size,
                                    Addr f)
{
   fl_post_reg_write(/*dummy*/0, tid, offset, size);
}
...
/* Look at the definedness of the guest's shadow state for 
   ...
   a parameter error.
*/
static void fl_pre_reg_read ( CorePart part, ThreadId tid, Char* s, 
                              OffT offset, SizeT size)
{
   Int   i;
   Bool  bad;
   ...
   if (bad)
      fl_record_regparam_error ( tid, s );
}
...
typedef struct _FL_Error FL_Error;
...
static void fl_pp_AddrInfo ( Addr a, AddrInfo* ai, Bool maybe_gcc )
{
   ...
   switch (ai->tag) {
      ...
      default:
         VG_(tool_panic)("fl_pp_AddrInfo");
   }
}

static void fl_pp_msg( Char* xml_name, Error* err, const HChar* format, ... )
{
...
static void fl_pp_Error ( Error* err )
{
   FL_Error* extra = VG_(get_error_extra)(err);
   ...
   switch (VG_(get_error_kind)(err)) {
      ...
      case Err_Value:
         fl_pp_msg("TaintedValue", err,
                   "Use of tainted value of size %d",
                   extra->Err.Value.szB);
         break;

      case Err_Cond:
         fl_pp_msg("TaintedCondition", err,
                   "Conditional jump or move depends"
                   " on tainted value(s)");
         break;

      case Err_RegParam:
         fl_pp_msg("SyscallParam", err,
                   "Syscall param %s contains tainted byte(s)",
                   VG_(get_error_string)(err));
         break;
      ...
      case Err_MemParam:
         fl_pp_msg("SyscallParam", err,
                   ...
                   ( extra->Err.MemParam.isAddrErr 
                     ? "unaddressable" : "tainted" ));
         fl_pp_AddrInfo(VG_(get_error_address)(err),
                        &extra->Err.MemParam.ai, False);
         break;

      case Err_User:
         fl_pp_msg("ClientCheck", err,
                   "%s byte(s) found during client check request", 
                   ( extra->Err.User.isAddrErr
                     ? "Unaddressable" : "Tainted" ));
         fl_pp_AddrInfo(VG_(get_error_address)(err), &extra->Err.User.ai,
                        False);
         break;

      case Err_Free:
         fl_pp_msg("InvalidFree", err,
      ...
      case Err_Addr:
         if (extra->Err.Addr.isWrite) {
            fl_pp_msg("InvalidWrite", err,
                      "Invalid write of size %d", 
                      extra->Err.Addr.szB); 
         } else {
            fl_pp_msg("InvalidRead", err,
                      "Invalid read of size %d", 
                      extra->Err.Addr.szB); 
         }
         fl_pp_AddrInfo(VG_(get_error_address)(err), &extra->Err.Addr.ai,
                        extra->Err.Addr.maybe_gcc);
         break;

      case Err_Jump:
         fl_pp_msg("InvalidJump", err,
                   "Jump to the invalid address stated on the next line");
         fl_pp_AddrInfo(VG_(get_error_address)(err), &extra->Err.Jump.ai,
                        False);
         break;

      case Err_Overlap:
         if (extra->Err.Overlap.szB == 0)
            fl_pp_msg("Overlap", err,
      ...
      default: 
         VG_(printf)("Error:\n  unknown Flayer error code %d\n",
                     VG_(get_error_kind)(err));
         VG_(tool_panic)("unknown error code in fl_pp_Error)");
   }
}
...
static void fl_record_address_error ( ThreadId tid, Addr a, Int szB,
                                      Bool isWrite )
{
   FL_Error extra;
   Bool     just_below_esp;

   /* XXX: DISABLED IN FLAYER.
    * Later determine if addressability has any bearing. */
   return;
...
#  if defined(VGP_ppc32_aix5) || defined(VGP_ppc64_aix5)
   /* AIX zero-page handling.  On AIX, reads from page zero are,
      ...
      though.  Since flayer can't distinguish reads from writes, the
...
#  endif
   ...
   if (FL_(clo_workaround_gcc296_bugs) && just_below_esp)
      return;
...
static void fl_record_value_error ( ThreadId tid, Int szB )
{
   FL_Error extra;
...
static void fl_record_cond_error ( ThreadId tid )
{
...
static void fl_record_core_mem_error ( ThreadId tid, Bool isAddrErr, Char* msg )
{
...
void FL_(record_free_error) ( ThreadId tid, Addr a ) 
{
   FL_Error extra;
   tl_assert(VG_INVALID_THREADID != tid);
...
void FL_(record_freemismatch_error) ( ThreadId tid, FL_Chunk* mc )
{
   FL_Error extra;
...
void FL_(record_illegal_mempool_error) ( ThreadId tid, Addr a ) 
{
   FL_Error extra;
   tl_assert(VG_INVALID_THREADID != tid);
...
static void fl_record_overlap_error ( ThreadId tid, Char* function,
...
static void fl_record_user_error ( ThreadId tid, Addr a, Bool isAddrErr )
{
   FL_Error extra;
...
static Bool fl_eq_Error ( VgRes res, Error* e1, Error* e2 )
{
   FL_Error* extra1 = VG_(get_error_extra)(e1);
   FL_Error* extra2 = VG_(get_error_extra)(e2);
   ...
   switch (VG_(get_error_kind)(e1)) {
      ...
      default: 
         ...
         VG_(tool_panic)("unknown error code in fl_eq_Error");
   }
}

/* Function used when searching FL_Chunk lists */
static Bool addr_is_in_FL_Chunk(FL_Chunk* mc, Addr a)
{
   // Nb: this is not quite right!  It assumes that the heap block has
   // a redzone of size FL_MALLOC_REDZONE_SZB.  That's true for malloc'd
   ...
   // Fixing would require adding redzone size to FL_Chunks, though.
   return VG_(addr_is_in_block)( a, mc->data, mc->szB,
                                 FL_MALLOC_REDZONE_SZB );
}
...
static void describe_addr ( Addr a, AddrInfo* ai )
{
   ...
   mc = FL_(get_freed_list_head)();
   while (mc) {
      if (addr_is_in_FL_Chunk(mc, a)) {
         ai->tag = Addr_Block;
...
static UInt fl_update_extra( Error* err )
{
   FL_Error* extra = VG_(get_error_extra)(err);
   ...
   // possible because we have the FL_Chunk on hand when the error is
   ...
   default: VG_(tool_panic)("fl_update_extra: bad errkind");
   }
}
...
static Bool fl_recognised_suppression ( Char* name, Supp* su )
{
   SuppKind skind;
...
static 
Bool fl_read_extra_suppression_info ( Int fd, Char* buf, Int nBuf, Supp *su )
{
   Bool eof;
...
static Bool fl_error_matches_suppression(Error* err, Supp* su)
{
   Int       su_szB;
   FL_Error* extra = VG_(get_error_extra)(err);
   ...
   switch (VG_(get_supp_kind)(su)) {
      ...
      default:
         ...
         VG_(tool_panic)("unknown suppression type in "
                         "FL_(error_matches_suppression)");
   }
}

static Char* fl_get_error_name ( Error* err )
{
   switch (VG_(get_error_kind)(err)) {
...
static void fl_print_extra_suppression_info ( Error* err )
{
   ErrorKind ekind = VG_(get_error_kind )(err);
...
static INLINE
ULong fl_LOADV64 ( Addr a, Bool isBigEndian )
{
   UWord   sm_off16, vabits16;
   SecMap* sm;

   PROF_EVENT(200, "fl_LOADV64");

#ifndef PERF_FAST_LOADV
   return fl_LOADVn_slow( a, 64, isBigEndian );
#else
   if (EXPECTED_NOT_TAKEN( UNALIGNED_OR_HIGH(a,64) )) {
      PROF_EVENT(201, "fl_LOADV64-slow1");
      return (ULong)fl_LOADVn_slow( a, 64, isBigEndian );
   }
   ...
   if (EXPECTED_TAKEN(vabits16 == VA_BITS16_UNTAINTED)) {
      return V_BITS64_UNTAINTED;
   } else if (EXPECTED_TAKEN(vabits16 == VA_BITS16_TAINTED)) {
      return V_BITS64_TAINTED;
   } else {
      /* Slow case: the 8 bytes are not all-defined or all-undefined. */
      PROF_EVENT(202, "fl_LOADV64-slow2");
      return fl_LOADVn_slow( a, 64, isBigEndian );
   }
#endif
}

VG_REGPARM(1) ULong FL_(helperc_LOADV64be) ( Addr a )
{
   return fl_LOADV64(a, True);
}
VG_REGPARM(1) ULong FL_(helperc_LOADV64le) ( Addr a )
{
   return fl_LOADV64(a, False);
}


static INLINE
void fl_STOREV64 ( Addr a, ULong vbits64, Bool isBigEndian )
{
   UWord   sm_off16, vabits16;
   SecMap* sm;

   PROF_EVENT(210, "fl_STOREV64");

#ifndef PERF_FAST_STOREV
   // XXX: this slow case seems to be marginally faster than the fast case!
   // Investigate further.
   fl_STOREVn_slow( a, 64, vbits64, isBigEndian );
#else
   if (EXPECTED_NOT_TAKEN( UNALIGNED_OR_HIGH(a,64) )) {
      PROF_EVENT(211, "fl_STOREV64-slow1");
      fl_STOREVn_slow( a, 64, vbits64, isBigEndian );
      return;
   }
                        ...
                        VA_BITS16_TAINTED == vabits16) ))
   {
      ...
      if (V_BITS64_UNTAINTED == vbits64) {
         ...
         PROF_EVENT(212, "fl_STOREV64-slow2");
         fl_STOREVn_slow( a, 64, vbits64, isBigEndian );
      }
   } else {
      /* Slow but general case. */
      PROF_EVENT(213, "fl_STOREV64-slow3");
      fl_STOREVn_slow( a, 64, vbits64, isBigEndian );
   }
#endif
}

VG_REGPARM(1) void FL_(helperc_STOREV64be) ( Addr a, ULong vbits64 )
{
   fl_STOREV64(a, vbits64, True);
}
VG_REGPARM(1) void FL_(helperc_STOREV64le) ( Addr a, ULong vbits64 )
{
   fl_STOREV64(a, vbits64, False);
}
...
static INLINE
UWord fl_LOADV32 ( Addr a, Bool isBigEndian )
{
   UWord   sm_off, vabits8;
   SecMap* sm;

   PROF_EVENT(220, "fl_LOADV32");

#ifndef PERF_FAST_LOADV
   return (UWord)fl_LOADVn_slow( a, 32, isBigEndian );
#else
   if (EXPECTED_NOT_TAKEN( UNALIGNED_OR_HIGH(a,32) )) {
      PROF_EVENT(221, "fl_LOADV32-slow1");
      return (UWord)fl_LOADVn_slow( a, 32, isBigEndian );
   }
   ...
   if (EXPECTED_TAKEN(vabits8 == VA_BITS8_UNTAINTED)) {
      return ((UWord)0xFFFFFFFF00000000ULL | (UWord)V_BITS32_UNTAINTED);
   } else if (EXPECTED_TAKEN(vabits8 == VA_BITS8_TAINTED)) {
      return ((UWord)0xFFFFFFFF00000000ULL | (UWord)V_BITS32_TAINTED);
   } else {
      /* Slow case: the 4 bytes are not all-defined or all-undefined. */
      PROF_EVENT(222, "fl_LOADV32-slow2");
      return (UWord)fl_LOADVn_slow( a, 32, isBigEndian );
   }
#endif
}

VG_REGPARM(1) UWord FL_(helperc_LOADV32be) ( Addr a )
{
   return fl_LOADV32(a, True);
}
VG_REGPARM(1) UWord FL_(helperc_LOADV32le) ( Addr a )
{
   return fl_LOADV32(a, False);
}


static INLINE
void fl_STOREV32 ( Addr a, UWord vbits32, Bool isBigEndian )
{
   UWord   sm_off, vabits8;
   SecMap* sm;

   PROF_EVENT(230, "fl_STOREV32");

#ifndef PERF_FAST_STOREV
   fl_STOREVn_slow( a, 32, (ULong)vbits32, isBigEndian );
#else
   if (EXPECTED_NOT_TAKEN( UNALIGNED_OR_HIGH(a,32) )) {
      PROF_EVENT(231, "fl_STOREV32-slow1");
      fl_STOREVn_slow( a, 32, (ULong)vbits32, isBigEndian );
      return;
   }
...
//---------------------------------------------------------------------------
   ...
   if (V_BITS32_UNTAINTED == vbits32) {
      if (vabits8 == (UInt)VA_BITS8_UNTAINTED) {
         ...
         PROF_EVENT(232, "fl_STOREV32-slow2");
         fl_STOREVn_slow( a, 32, (ULong)vbits32, isBigEndian );
      }
      ...
      if (vabits8 == (UInt)VA_BITS8_TAINTED) {
         ...
         // not defined/undefined, or distinguished and changing state
         PROF_EVENT(233, "fl_STOREV32-slow3");
         fl_STOREVn_slow( a, 32, (ULong)vbits32, isBigEndian );
      }
   } else {
      // Partially defined word
      PROF_EVENT(234, "fl_STOREV32-slow4");
      fl_STOREVn_slow( a, 32, (ULong)vbits32, isBigEndian );
   }
//---------------------------------------------------------------------------
#else
...
VG_REGPARM(2) void FL_(helperc_STOREV32be) ( Addr a, UWord vbits32 )
{
   fl_STOREV32(a, vbits32, True);
}
VG_REGPARM(2) void FL_(helperc_STOREV32le) ( Addr a, UWord vbits32 )
{
   fl_STOREV32(a, vbits32, False);
}
...
static INLINE
UWord fl_LOADV16 ( Addr a, Bool isBigEndian )
{
   UWord   sm_off, vabits8;
   SecMap* sm;

   PROF_EVENT(240, "fl_LOADV16");

#ifndef PERF_FAST_LOADV
   return (UWord)fl_LOADVn_slow( a, 16, isBigEndian );
#else
   if (EXPECTED_NOT_TAKEN( UNALIGNED_OR_HIGH(a,16) )) {
      PROF_EVENT(241, "fl_LOADV16-slow1");
      return (UWord)fl_LOADVn_slow( a, 16, isBigEndian );
   }
   ...
   if      (vabits8 == VA_BITS8_UNTAINTED  ) { return V_BITS16_UNTAINTED;   }
      ...
      if      (vabits4 == VA_BITS4_UNTAINTED  ) { return V_BITS16_UNTAINTED;   }
         ...
         PROF_EVENT(242, "fl_LOADV16-slow2");
         return (UWord)fl_LOADVn_slow( a, 16, isBigEndian );
      }
   }
#endif
...
VG_REGPARM(1) UWord FL_(helperc_LOADV16be) ( Addr a )
{
   return fl_LOADV16(a, True);
}
VG_REGPARM(1) UWord FL_(helperc_LOADV16le) ( Addr a )
{
   return fl_LOADV16(a, False);
}


static INLINE
void fl_STOREV16 ( Addr a, UWord vbits16, Bool isBigEndian )
{
   UWord   sm_off, vabits8;
   SecMap* sm;

   PROF_EVENT(250, "fl_STOREV16");

#ifndef PERF_FAST_STOREV
   fl_STOREVn_slow( a, 16, (ULong)vbits16, isBigEndian );
#else
   if (EXPECTED_NOT_TAKEN( UNALIGNED_OR_HIGH(a,16) )) {
      PROF_EVENT(251, "fl_STOREV16-slow1");
      fl_STOREVn_slow( a, 16, (ULong)vbits16, isBigEndian );
      return;
   }
                        ...
                        VA_BITS8_TAINTED == vabits8) ))
   {
      ...
      if (V_BITS16_UNTAINTED == vbits16) {
         ...
         /* Slow but general case -- writing partially defined bytes. */
         PROF_EVENT(252, "fl_STOREV16-slow2");
         fl_STOREVn_slow( a, 16, (ULong)vbits16, isBigEndian );
      }
   } else {
      /* Slow but general case. */
      PROF_EVENT(253, "fl_STOREV16-slow3");
      fl_STOREVn_slow( a, 16, (ULong)vbits16, isBigEndian );
   }
#endif
}

VG_REGPARM(2) void FL_(helperc_STOREV16be) ( Addr a, UWord vbits16 )
{
   fl_STOREV16(a, vbits16, True);
}
VG_REGPARM(2) void FL_(helperc_STOREV16le) ( Addr a, UWord vbits16 )
{
   fl_STOREV16(a, vbits16, False);
}
...
VG_REGPARM(1)
UWord FL_(helperc_LOADV8) ( Addr a )
{
   UWord   sm_off, vabits8;
   SecMap* sm;

   PROF_EVENT(260, "fl_LOADV8");

#ifndef PERF_FAST_LOADV
   return (UWord)fl_LOADVn_slow( a, 8, False/*irrelevant*/ );
#else
   if (EXPECTED_NOT_TAKEN( UNALIGNED_OR_HIGH(a,8) )) {
      PROF_EVENT(261, "fl_LOADV8-slow1");
      return (UWord)fl_LOADVn_slow( a, 8, False/*irrelevant*/ );
   }
   ...
   if      (vabits8 == VA_BITS8_UNTAINTED  ) { return V_BITS8_UNTAINTED;   }
      ...
      if      (vabits2 == VA_BITS2_UNTAINTED  ) { return V_BITS8_UNTAINTED;   }
         ...
         PROF_EVENT(262, "fl_LOADV8-slow2");
         return (UWord)fl_LOADVn_slow( a, 8, False/*irrelevant*/ );
      }
   }
#endif
...
VG_REGPARM(2)
void FL_(helperc_STOREV8) ( Addr a, UWord vbits8 )
{
   UWord   sm_off, vabits8;
   SecMap* sm;

   PROF_EVENT(270, "fl_STOREV8");

#ifndef PERF_FAST_STOREV
   fl_STOREVn_slow( a, 8, (ULong)vbits8, False/*irrelevant*/ );
#else
   if (EXPECTED_NOT_TAKEN( UNALIGNED_OR_HIGH(a,8) )) {
      PROF_EVENT(271, "fl_STOREV8-slow1");
      fl_STOREVn_slow( a, 8, (ULong)vbits8, False/*irrelevant*/ );
      return;
   }
      ...
      )
   {
      ...
      if (V_BITS8_UNTAINTED == vbits8) {
         ...
         /* Slow but general case -- writing partially defined bytes. */
         PROF_EVENT(272, "fl_STOREV8-slow2");
         fl_STOREVn_slow( a, 8, (ULong)vbits8, False/*irrelevant*/ );
      }
   } else {
      /* Slow but general case. */
      PROF_EVENT(273, "fl_STOREV8-slow3");
      fl_STOREVn_slow( a, 8, (ULong)vbits8, False/*irrelevant*/ );
   }
#endif
}
...
void FL_(helperc_value_check0_fail) ( void )
{
   fl_record_cond_error ( VG_(get_running_tid)() );
}

void FL_(helperc_value_check1_fail) ( void )
{
   fl_record_value_error ( VG_(get_running_tid)(), 1 );
}

void FL_(helperc_value_check4_fail) ( void )
{
   fl_record_value_error ( VG_(get_running_tid)(), 4 );
}

void FL_(helperc_value_check8_fail) ( void )
{
   fl_record_value_error ( VG_(get_running_tid)(), 8 );
}

VG_REGPARM(1) void FL_(helperc_complain_undef) ( HWord sz )
{
   fl_record_value_error ( VG_(get_running_tid)(), (Int)sz );
}
...
static Int fl_get_or_set_vbits_for_client ( 
   ThreadId tid,
   Addr a, 
   Addr vbits, 
   SizeT szB, 
...
)
{
   ...
   if (setting) {
      ...
      FL_(make_mem_defined)(vbits, szB);
   }
...
static Bool fl_cheap_sanity_check ( void )
{
...
static Bool fl_expensive_sanity_check ( void )
{
   Int     i;
   ...
   sm = &sm_distinguished[SM_DIST_TAINTED];
   for (i = 0; i < SM_CHUNKS; i++)
      if (sm->vabits8[i] != VA_BITS8_TAINTED)
         bad = True;

   /* Check defined DSM. */
   sm = &sm_distinguished[SM_DIST_UNTAINTED];
   for (i = 0; i < SM_CHUNKS; i++)
      if (sm->vabits8[i] != VA_BITS8_UNTAINTED)
         bad = True;

   if (bad) {
      VG_(printf)("flayer expensive sanity: "
   ...
   if (errmsg) {
      VG_(printf)("flayer expensive sanity, auxmaps:\n\t%s", errmsg);
      return False;
   }
...
Bool          FL_(clo_partial_loads_ok)       = False;
Int           FL_(clo_freelist_vol)           = 5000000;
Bool          FL_(clo_workaround_gcc296_bugs) = False;
Char*         FL_(clo_alter_branch)           = NULL;
Char*         FL_(clo_alter_fn)                = NULL;
Char*         FL_(clo_taint_string)           = NULL;
static Char   FL_(default_file_filter)[] = "";
Char*         FL_(clo_file_filter)            = FL_(default_file_filter);
Bool          FL_(clo_taint_file)             = False;
Bool          FL_(clo_taint_network)          = False;
Bool          FL_(clo_taint_stdin)            = False;
Bool          FL_(clo_verbose_instr)          = False;

static Bool fl_process_cmd_line_options(Char* arg)
{
	VG_BOOL_CLO(arg, "--partial-loads-ok",      FL_(clo_partial_loads_ok))
   else VG_BOOL_CLO(arg, "--workaround-gcc296-bugs",FL_(clo_workaround_gcc296_bugs))

   else VG_STR_CLO(arg, "--alter-branch", FL_(clo_alter_branch))
   else VG_STR_CLO(arg, "--alter-fn", FL_(clo_alter_fn))
   else VG_STR_CLO(arg, "--taint-string", FL_(clo_taint_string))
   else VG_STR_CLO(arg, "--file-filter", FL_(clo_file_filter))
   else VG_BOOL_CLO(arg, "--taint-stdin", FL_(clo_taint_stdin))
   else VG_BOOL_CLO(arg, "--taint-file", FL_(clo_taint_file))
   else VG_BOOL_CLO(arg, "--taint-network", FL_(clo_taint_network))
   else VG_BOOL_CLO(arg, "--verbose-instrumentation", FL_(clo_verbose_instr))
   
   else VG_BNUM_CLO(arg, "--freelist-vol",  FL_(clo_freelist_vol), 0, 1000000000)
...
static void fl_print_usage(void)
{  
   VG_(printf)(
...
static void fl_print_debug_usage(void)
{  
...
static Bool client_perm_maybe_describe( Addr a, AddrInfo* ai )
{
   ...
   for (i = 0; i < cgb_used; i++) {
      ...
      if (VG_(addr_is_in_block)(a, cgbs[i].start, cgbs[i].size, 0)) {
         ...
         FL_Mempool* mp = VG_(HT_lookup)(FL_(mempool_list),
                                          (UWord)cgbs[i].start);
         if (mp != NULL) {
            if (mp->chunks != NULL) {
               FL_Chunk* mc;
...
static Bool fl_handle_client_request ( ThreadId tid, UWord* arg, UWord* ret )
{
   Int   i;
   Bool  ok;
   Addr  bad_addr;

   if (!VG_IS_TOOL_USERREQ('F','L',arg[0])
   ...
   switch (arg[0]) {
      case VG_USERREQ__CHECK_MEM_IS_ADDRESSABLE:
         ...
         if (!ok)
            fl_record_user_error ( tid, bad_addr, /*isAddrErr*/True );
         *ret = ok ? (UWord)NULL : bad_addr;
         break;

      case VG_USERREQ__CHECK_MEM_IS_UNTAINTED: {
         FL_ReadResult res;
         res = is_mem_defined ( arg[1], arg[2], &bad_addr );
         if (FL_AddrErr == res)
            fl_record_user_error ( tid, bad_addr, /*isAddrErr*/True );
         else if (FL_ValueErr == res)
            fl_record_user_error ( tid, bad_addr, /*isAddrErr*/False );
         *ret = ( res==FL_Ok ? (UWord)NULL : bad_addr );
         break;
      }

      case VG_USERREQ__MAKE_MEM_NOACCESS:
         FL_(make_mem_noaccess) ( arg[1], arg[2] );
         *ret = -1;
         break;

      case VG_USERREQ__MAKE_MEM_TAINTED:
         FL_(make_mem_undefined) ( arg[1], arg[2] );
         *ret = -1;
         break;

      case VG_USERREQ__MAKE_MEM_UNTAINTED:
         FL_(make_mem_defined) ( arg[1], arg[2] );
         *ret = -1;
         break;

      case VG_USERREQ__MAKE_MEM_UNTAINTED_IF_ADDRESSABLE:
      ...
      case VG_USERREQ__GET_VBITS:
         *ret = fl_get_or_set_vbits_for_client
      ...
      case VG_USERREQ__MALLOCLIKE_BLOCK: {
         ...
         FL_(new_block) ( tid, p, sizeB, /*ignored*/0, rzB, is_zeroed, 
                          FL_AllocCustom, FL_(malloc_list) );
         return True;
      }
      case VG_USERREQ__FREELIKE_BLOCK: {
         ...
         FL_(handle_free) ( tid, p, rzB, FL_AllocCustom );
         return True;
      }

      case _VG_USERREQ__FLAYER_RECORD_OVERLAP_ERROR: {
         Char* s   = (Char*)arg[1];
         Addr  dst = (Addr) arg[2];
         Addr  src = (Addr) arg[3];
         SizeT len = (SizeT)arg[4];
         fl_record_overlap_error(tid, s, src, dst, len);
         return True;
      }
      ...
      case VG_USERREQ__CREATE_MEMPOOL: {
         ...
         FL_(create_mempool) ( pool, rzB, is_zeroed );
         return True;
      }
      ...
      case VG_USERREQ__MEMPOOL_ALLOC: {
         ...
         FL_(mempool_alloc) ( tid, pool, addr, size );
         return True;
      }
      ...
      case VG_USERREQ__MEMPOOL_TRIM: {
         ...
         FL_(mempool_trim) ( pool, addr, size );
         return True;
      }
      ...
      case VG_USERREQ__MEMPOOL_CHANGE: {
         ...
         FL_(mempool_change) ( pool, addrA, addrB, size );
         return True;
      }
      ...
      case VG_USERREQ__MEMPOOL_EXISTS: {
         ...
         *ret = (UWord) FL_(mempool_exists) ( pool );
	 return True;
      }
      ...
      default:
         VG_(message)(Vg_UserMsg, 
                      "Warning: unknown flayer client request code %llx",
                      (ULong)arg[0]);
         return False;
   }
   return True;
...
// if FL_PROFILE_MEMORY is defined.

#ifdef FL_PROFILE_MEMORY

UInt   FL_(event_ctr)[N_PROF_EVENTS];
HChar* FL_(event_ctr_name)[N_PROF_EVENTS];
...
static void done_prof_mem ( void )
{
   ...
   for (i = 0; i < N_PROF_EVENTS; i++) {
      ...
      if (FL_(event_ctr)[i] > 0) {
         spaced = False;
         VG_(printf)( "prof mem event %3d: %9d   %s\n", 
                      i, FL_(event_ctr)[i],
                      FL_(event_ctr_name)[i] 
                         ? FL_(event_ctr_name)[i] : "unnamed");
      }
   }
}
...
static void fl_post_clo_init ( void )
{
}
...
static void fl_fini ( Int exitcode )
{
   FL_(print_malloc_stats)();
   ...
   if (VG_(clo_verbosity) > 1) {
      ...
      VG_(message)(Vg_DebugMsg,
         " flayer: sanity checks: %d cheap, %d expensive",
         n_sanity_cheap, n_sanity_expensive );
      VG_(message)(Vg_DebugMsg,
         " flayer: auxmaps: %d auxmap entries (%dk, %dM) in use",
         n_auxmap_L2_nodes, 
         n_auxmap_L2_nodes * 64, 
         n_auxmap_L2_nodes / 16 );
      VG_(message)(Vg_DebugMsg,
         " flayer: auxmaps_L1: %lld searches, %lld cmps, ratio %lld:10",
         n_auxmap_L1_searches, n_auxmap_L1_cmps,
         (10ULL * n_auxmap_L1_cmps) 
            / (n_auxmap_L1_searches ? n_auxmap_L1_searches : 1) 
      );   
      VG_(message)(Vg_DebugMsg,
         " flayer: auxmaps_L2: %lld searches, %lld nodes",
      ...
      VG_(message)(Vg_DebugMsg,
         " flayer: max sec V bit nodes:    %d (%dk, %dM)",
         max_secVBit_nodes, max_secVBit_szB / 1024,
                            max_secVBit_szB / (1024 * 1024));
      VG_(message)(Vg_DebugMsg,
         " flayer: set_sec_vbits8 calls: %llu (new: %llu, updates: %llu)",
         sec_vbits_new_nodes + sec_vbits_updates,
         sec_vbits_new_nodes, sec_vbits_updates );
      VG_(message)(Vg_DebugMsg,
         " flayer: max shadow mem size:   %dk, %dM",
...
static void fl_pre_clo_init(void)
{
   VG_(details_name)            ("Flayer");
   VG_(details_version)         (NULL);
   VG_(details_description)     ("a input tracer and branch alterer");
   VG_(details_copyright_author)(
      "Copyright (C) 2007, and GNU GPL'd, by Will Drewry (Google Inc).");
   VG_(details_bug_reports_to)  (VG_BUGS_TO);
   VG_(details_avg_translation_sizeB) ( 556 );

   VG_(basic_tool_funcs)          (fl_post_clo_init,
                                   FL_(instrument),
                                   fl_fini);

   VG_(needs_core_errors)         ();
   VG_(needs_tool_errors)         (fl_eq_Error,
                                   fl_pp_Error,
                                   fl_update_extra,
                                   fl_recognised_suppression,
                                   fl_read_extra_suppression_info,
                                   fl_error_matches_suppression,
                                   fl_get_error_name,
                                   fl_print_extra_suppression_info);
   VG_(needs_libc_freeres)        ();
   VG_(needs_command_line_options)(fl_process_cmd_line_options,
                                   fl_print_usage,
                                   fl_print_debug_usage);
   VG_(needs_client_requests)     (fl_handle_client_request);
   VG_(needs_sanity_checks)       (fl_cheap_sanity_check,
                                   fl_expensive_sanity_check);
   VG_(needs_malloc_replacement)  (FL_(malloc),
                                   FL_(__builtin_new),
                                   FL_(__builtin_vec_new),
                                   FL_(memalign),
                                   FL_(calloc),
                                   FL_(free),
                                   FL_(__builtin_delete),
                                   FL_(__builtin_vec_delete),
                                   FL_(realloc),
                                   FL_MALLOC_REDZONE_SZB );
   VG_(needs_xml_output)          ();

   // when using undef as taint, nothing new should be undefined
   VG_(track_new_mem_stack_signal)( FL_(make_mem_defined) );
   VG_(track_new_mem_brk)         ( FL_(make_mem_defined) );

   VG_(track_new_mem_startup)     ( fl_new_mem_startup );
   VG_(track_new_mem_stack_signal)( FL_(make_mem_defined) );
   VG_(track_new_mem_brk)         ( FL_(make_mem_defined) );
   VG_(track_new_mem_mmap)        ( fl_new_mem_mmap );
   
   VG_(track_copy_mem_remap)      ( FL_(copy_address_range_state) );
   ...
   VG_(track_die_mem_stack_signal)( FL_(make_mem_noaccess) ); 
   VG_(track_die_mem_brk)         ( FL_(make_mem_noaccess) );
   VG_(track_die_mem_munmap)      ( FL_(make_mem_noaccess) ); 

#ifdef PERF_FAST_STACK
   VG_(track_new_mem_stack_4)     ( fl_new_mem_stack_4   );
   VG_(track_new_mem_stack_8)     ( fl_new_mem_stack_8   );
   VG_(track_new_mem_stack_12)    ( fl_new_mem_stack_12  );
   VG_(track_new_mem_stack_16)    ( fl_new_mem_stack_16  );
   VG_(track_new_mem_stack_32)    ( fl_new_mem_stack_32  );
   VG_(track_new_mem_stack_112)   ( fl_new_mem_stack_112 );
   VG_(track_new_mem_stack_128)   ( fl_new_mem_stack_128 );
   VG_(track_new_mem_stack_144)   ( fl_new_mem_stack_144 );
   VG_(track_new_mem_stack_160)   ( fl_new_mem_stack_160 );
#endif
   VG_(track_new_mem_stack)       ( fl_new_mem_stack     );

#ifdef PERF_FAST_STACK
   VG_(track_die_mem_stack_4)     ( fl_die_mem_stack_4   );
   VG_(track_die_mem_stack_8)     ( fl_die_mem_stack_8   );
   VG_(track_die_mem_stack_12)    ( fl_die_mem_stack_12  );
   VG_(track_die_mem_stack_16)    ( fl_die_mem_stack_16  );
   VG_(track_die_mem_stack_32)    ( fl_die_mem_stack_32  );
   VG_(track_die_mem_stack_112)   ( fl_die_mem_stack_112 );
   VG_(track_die_mem_stack_128)   ( fl_die_mem_stack_128 );
   VG_(track_die_mem_stack_144)   ( fl_die_mem_stack_144 );
   VG_(track_die_mem_stack_160)   ( fl_die_mem_stack_160 );
#endif
   VG_(track_die_mem_stack)       ( fl_die_mem_stack     );
   
   VG_(track_ban_mem_stack)       ( FL_(make_mem_noaccess) );

   VG_(track_pre_mem_read)        ( check_mem_is_defined );
   VG_(track_pre_mem_read_asciiz) ( check_mem_is_defined_asciiz );
   // flayer doesn't really care about addressability but having the
   // information may be useful later.
   VG_(track_pre_mem_write)       ( check_mem_is_addressable );
   VG_(track_post_mem_write)      ( fl_post_mem_write );

   VG_(track_pre_reg_read)     ( fl_pre_reg_read );

   VG_(track_post_reg_write)                  ( fl_post_reg_write );
   VG_(track_post_reg_write_clientcall_return)( fl_post_reg_write_clientcall );

   /* Track syscalls for tainting purposes */
   // TODO: will this conflict?
   VG_(needs_syscall_wrapper)     ( fl_pre_syscall,
                                    fl_post_syscall );


   init_shadow_memory();
   FL_(malloc_list)  = VG_(HT_construct)( 80021 );   // prime, big
   FL_(mempool_list) = VG_(HT_construct)( 1009  );   // prime, not so big
   FL_(setup_tainted_map)();
   /* XXX: this will go away when ThreadState is fully used */
   FL_(setup_guest_args)();
   init_prof_mem();

   tl_assert( fl_expensive_sanity_check() );
...
VG_DETERMINE_INTERFACE_VERSION(fl_pre_clo_init)
