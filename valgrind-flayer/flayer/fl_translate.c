...
/*
   ...
   This file was originally part of MemCheck.
...
/* Carries around state during flayer instrumentation. */
typedef
   struct _MCEnv {
...
static IRType shadowType ( IRType ty )
{
   switch (ty) {
      ...
      default: ppIRType(ty); 
               VG_(tool_panic)("flayer:shadowType");
   }
}
...
static IRExpr* definedOfType ( IRType ty ) {
   switch (ty) {
      ...
      default:       VG_(tool_panic)("flayer:definedOfType");
   }
}
...
static void complainIfUndefined ( MCEnv* mce, IRAtom* atom )
{
   ...
   switch (sz) {
      case 0:
         di = unsafeIRDirty_0_N( 
                 0/*regparms*/, 
                 "FL_(helperc_value_check0_fail)",
                 VG_(fnptr_to_fnentry)( &FL_(helperc_value_check0_fail) ),
                 mkIRExprVec_0() 
              );
         break;
      case 1:
         di = unsafeIRDirty_0_N( 
                 0/*regparms*/, 
                 "FL_(helperc_value_check1_fail)",
                 VG_(fnptr_to_fnentry)( &FL_(helperc_value_check1_fail) ),
                 mkIRExprVec_0() 
              );
         break;
      case 4:
         di = unsafeIRDirty_0_N( 
                 0/*regparms*/, 
                 "FL_(helperc_value_check4_fail)",
                 VG_(fnptr_to_fnentry)( &FL_(helperc_value_check4_fail) ),
                 mkIRExprVec_0() 
              );
         break;
      case 8:
         di = unsafeIRDirty_0_N( 
                 0/*regparms*/, 
                 "FL_(helperc_value_check8_fail)",
                 VG_(fnptr_to_fnentry)( &FL_(helperc_value_check8_fail) ),
                 mkIRExprVec_0() 
              );
         break;
      default:
         di = unsafeIRDirty_0_N( 
                 1/*regparms*/, 
                 "FL_(helperc_complain_undef)",
                 VG_(fnptr_to_fnentry)( &FL_(helperc_complain_undef) ),
   ...
   /* sameKindedAtoms ...*/
   /* XXX: does this make sense for tainting? 
    *      I think we want thsi behavior but I'm not sure if this
    *      will result in losing taint?
    */
...
static Bool isAlwaysDefd ( MCEnv* mce, Int offset, Int size )
{
   ...
   /* When dealing with tainting, no sections are considered always
    * defined.
    */
   for (i = 0; i < mce->layout->n_alwaysDefd; i++) {
      ...
      VG_(tool_panic)("flayer:isAlwaysDefd:partial overlap");
   }
#endif
                         ...
                         IRAtom* atom3, IRAtom* atom4 )
{
   ...
   switch (op) {
      ...
      default:
         ppIROp(op);
         VG_(tool_panic)("flayer:expr2vbits_Qop");
   }
}
                           ...
                           IRAtom* atom1, IRAtom* atom2, IRAtom* atom3 )
{
   ...
   switch (op) {
      ...
      default:
         ppIROp(op);
         VG_(tool_panic)("flayer:expr2vbits_Triop");
   }
}
                           ...
                           IRAtom* atom1, IRAtom* atom2 )
{
   ...
   switch (op) {
	 ...
	 return at;
      ...
      default:
         ppIROp(op);
         VG_(tool_panic)("flayer:expr2vbits_Binop");
   }
}
...
IRExpr* expr2vbits_Unop ( MCEnv* mce, IROp op, IRAtom* atom )
{
   ...
   switch (op) {
      ...
      default:
         ppIROp(op);
         VG_(tool_panic)("flayer:expr2vbits_Unop");
   }
}
                              ...
                              IRAtom* addr, UInt bias )
{
   ...
   if (end == Iend_LE) {   
      switch (ty) {
         case Ity_I64: helper = &FL_(helperc_LOADV64le);
                       hname = "FL_(helperc_LOADV64le)";
                       break;
         case Ity_I32: helper = &FL_(helperc_LOADV32le);
                       hname = "FL_(helperc_LOADV32le)";
                       break;
         case Ity_I16: helper = &FL_(helperc_LOADV16le);
                       hname = "FL_(helperc_LOADV16le)";
                       break;
         case Ity_I8:  helper = &FL_(helperc_LOADV8);
                       hname = "FL_(helperc_LOADV8)";
                       break;
         default:      ppIRType(ty);
                       VG_(tool_panic)("flayer:do_shadow_Load(LE)");
      }
   } else {
      switch (ty) {
         case Ity_I64: helper = &FL_(helperc_LOADV64be);
                       hname = "FL_(helperc_LOADV64be)";
                       break;
         case Ity_I32: helper = &FL_(helperc_LOADV32be);
                       hname = "FL_(helperc_LOADV32be)";
                       break;
         case Ity_I16: helper = &FL_(helperc_LOADV16be);
                       hname = "FL_(helperc_LOADV16be)";
                       break;
         case Ity_I8:  helper = &FL_(helperc_LOADV8);
                       hname = "FL_(helperc_LOADV8)";
                       break;
         default:      ppIRType(ty);
                       VG_(tool_panic)("flayer:do_shadow_Load(BE)");
      }
   }
...
IRExpr* expr2vbits ( MCEnv* mce, IRExpr* e )
{
   switch (e->tag) {
      ...
      default: 
         ...
         ppIRExpr(e);
         VG_(printf)("\n");
         VG_(tool_panic)("flayer: expr2vbits");
   }
}
                       ...
                       IRAtom* data, IRAtom* vdata )
{
   ...
   if (end == Iend_LE) {
      switch (ty) {
         ...
         case Ity_I64: helper = &FL_(helperc_STOREV64le);
                       hname = "FL_(helperc_STOREV64le)";
                       break;
         case Ity_I32: helper = &FL_(helperc_STOREV32le);
                       hname = "FL_(helperc_STOREV32le)";
                       break;
         case Ity_I16: helper = &FL_(helperc_STOREV16le);
                       hname = "FL_(helperc_STOREV16le)";
                       break;
         case Ity_I8:  helper = &FL_(helperc_STOREV8);
                       hname = "FL_(helperc_STOREV8)";
                       break;
         default:      VG_(tool_panic)("flayer:do_shadow_Store(LE)");
      }
   } else {
      switch (ty) {
         case Ity_V128: /* we'll use the helper twice */
         case Ity_I64: helper = &FL_(helperc_STOREV64be);
                       hname = "FL_(helperc_STOREV64be)";
                       break;
         case Ity_I32: helper = &FL_(helperc_STOREV32be);
                       hname = "FL_(helperc_STOREV32be)";
                       break;
         case Ity_I16: helper = &FL_(helperc_STOREV16be);
                       hname = "FL_(helperc_STOREV16be)";
                       break;
         case Ity_I8:  helper = &FL_(helperc_STOREV8);
                       hname = "FL_(helperc_STOREV8)";
                       break;
         default:      VG_(tool_panic)("flayer:do_shadow_Store(BE)");
      }
   }
...
static IRType szToITy ( Int n )
{
   switch (n) {
      ...
      default: VG_(tool_panic)("szToITy(flayer)");
   }
}
...
#  endif
   ...
   for (i = 0; i < d->nFxState; i++) {
      ...
      if (isAlwaysDefd(mce, d->fxState[i].offset, d->fxState[i].size )) {
         if (0)
         VG_(printf)("flayer: Dirty gst: ignored off %d, sz %d\n",
...
static
Bool addr_in_skip_list(Addr64 a, Int l, Long *ret) {
  Addr64 instr = a;
  Char addr[21], *loc;
  Char *end = FL_(clo_alter_fn) ? FL_(clo_alter_fn) + VG_(strlen)( FL_(clo_alter_fn) ) : NULL;

  if (FL_(clo_alter_fn) == NULL)
    return False;

  for ( ; instr < a+l; ++instr ) {
    VG_(snprintf)(addr, sizeof(addr), "%p", (HWord)instr);
    loc = VG_(strstr)(FL_(clo_alter_fn), addr);
    if (loc) {
      // Now skip past the address
      Char *value = loc + VG_(strlen)(addr) + 1;
      if (value < end) {
        *ret = VG_(atoll)(value);
        return True;
      }
...
void do_AbiHint ( MCEnv* mce, IRExpr* base, Int len )
{
   ...
   di = unsafeIRDirty_0_N(
           0/*regparms*/,
           "FL_(helperc_MAKE_STACK_UNINIT)",
           VG_(fnptr_to_fnentry)( &FL_(helperc_MAKE_STACK_UNINIT) ),
...
{
   Bool    verboze = FL_(clo_verbose_instr);
   Bool    bogus;
   Int     i, j, first_stmt;
   IRStmt* st;
   MCEnv   mce;
   ...
   Addr64 imark_addr = 0;
   Int  imark_len = 0;
   Char *instr_addr_p = NULL;
   Long skip_ret;
   // Used by alter-branch to track address of a Ist_Exit IMark.
   // TODO(redpig@dataspill.org): make this a define
   Char instr_addr[21];
   ...
   /* Nasty problem.  IR optimisation of the pre-instrumented IR may
      ...
      flayer to generate references to their shadows.  End effect is
   ...
   */
   ...
   for (/* use current i*/; i <  bb_in->stmts_used; i++) {
      ...
      switch (st->tag) {
         ...
         case Ist_WrTmp:
            /* We may need to hijack comparisions to do assignments */
#if 0
            if (FL_(clo_alter_branch) != NULL)
              instr_addr_p = VG_(strstr)( FL_(clo_alter_branch), instr_addr );

            // Extract the behavior if we have a match.
            if (instr_addr_p != NULL) {
              Char *end = FL_(clo_alter_branch) +
                VG_(strlen)( FL_(clo_alter_branch) );
              Char *behavior = instr_addr_p + VG_(strlen)( instr_addr ) + 1;
              if (behavior < end) {

                if (*behavior  == '0') { // 2 - XOR out CCalls
                } else if (*behavior  == '=') { // XXX: FINISH THIS
                  if (st->Ist.WrTmp.data->tag == Iex_Binop) {
                    if (st->Ist.WrTmp.data->Iex.Binop.op == Iop_CmpEQ32  ||
                        st->Ist.WrTmp.data->Iex.Binop.op == Iop_CmpEQ8) {
                      // XXX: ARG! cannot re-assign the tmp.  We need to update real memory.
                      // How do we trace it back? Search backwards for a PUT with t23?
                      // Roll backwards looking for the last PUT from t23 or the 

                      // Copy the IRExpr for the Put or create a new PUT if it was a load
                      // Create a new tmp and assign the value. Taint it for good measure.
                      // Assign this to Put
                      // XXX: add a complain() for this event.
                      //assign( bb, st->Ist.WrTmp.data->Iex.Binop.arg1->Iex.RdTmp.tmp,
                      //            st->Ist.WrTmp.data->Iex.Binop.arg2 );
                      if (st->Ist.WrTmp.data->Iex.Binop.arg1->tag == Iex_RdTmp) {
                        VG_(printf)("??? %d == %d\n",  st->Ist.WrTmp.data->Iex.Binop.arg1->Iex.RdTmp.tmp, st->Ist.WrTmp.data->Iex.Binop.arg2->Iex.RdTmp.tmp);
                        IRTemp tmp = st->Ist.WrTmp.data->Iex.Binop.arg1->Iex.RdTmp.tmp;
                        newShadowTmp(&mce, tmp);
                        assign(bb, findShadowTmp(&mce, tmp),
                               st->Ist.WrTmp.data->Iex.Binop.arg2);
                      }
                    }
                  }
         ...
         case Ist_Store:
          ...
          case Ist_Exit:
            // Always complain about tainted guards - even when we replace them.
            complainIfUndefined( &mce, st->Ist.Exit.guard );
            // The guard is the expression used to determine if
            // an Ist_Exit will be followed. By passing in
            // pointer:value pairs to change-branch, this will
            // force the value to true or false.

            if (FL_(clo_alter_branch) != NULL)
              instr_addr_p = VG_(strstr)( FL_(clo_alter_branch), instr_addr );
            //VG_(printf)("Looking for %s in %s\n",
            //  instr_addr, FL_(clo_alter_branch));
            ...
            if (instr_addr_p != NULL) {
              ...
              if (behavior < end) {
                if (*behavior  == '0') {
                  st->Ist.Exit.guard = mkU1(0);
                  //VG_(printf)("======= Setting branch (%s) guard: %d\n",
                  //  instr_addr, 0);
                } else if (*behavior  == '1') {
                  st->Ist.Exit.guard = mkU1(1);
                  //VG_(printf)("======= Setting branch (%s) guard: %d\n",
                  //  instr_addr, 1);
                }
              }
         ...
         case Ist_NoOp:
         case Ist_IMark:
            // Track the current address in %p format
            VG_(snprintf)(instr_addr, sizeof(instr_addr), "%p",
              (HWord)st->Ist.IMark.addr);
            /* Store these for function skipping */
           imark_addr = st->Ist.IMark.addr;
           imark_len = st->Ist.IMark.len;
         case Ist_MFence:
            break;
         ...
         default:
            ...
            ppIRStmt(st);
            VG_(printf)("\n");
            VG_(tool_panic)("flayer: unhandled IRStmt");
      ...
      addStmtToIRSB(bb, st);
       ...
       /* If --skip-function=addr:val,... is specified, check to see if the
        * addresses fall within the current instruction. If so, add a jump to
        * the exact next instruction, simulating skipping of the call action.
        * This _will_ not skip linked functions as those should be accessed via
        * bb->next and bb->jumpkind.  That will have to be handled separately.
       */

      // need to check inside the imark addr and len for a match to the addr.
      //instr_addr_p = VG_(strstr)( "0x80484C0", instr_addr ); // if.c hardcoded foo test
      if (i == bb_in->stmts_used-1 || bb_in->stmts[i+1]->tag == Ist_IMark) {
        if (addr_in_skip_list(imark_addr, imark_len, &skip_ret)) {
          // XXX: Assume addition is the right direction.
          IRStmt *jump = IRStmt_Exit(mkU1(1), Ijk_Boring, IRConst_U32(imark_addr + imark_len));
          /* Set EAX to the request return value */
          addStmtToIRSB(bb, IRStmt_Put(0, mkU32(skip_ret)));
          /* Now jump over the unwanted call */
          addStmtToIRSB(bb, jump);
          if (verboze) {
            VG_(printf)("Adding a jump:\n");
            ppIRStmt(jump);
            VG_(printf)("\n");
          }
        }
